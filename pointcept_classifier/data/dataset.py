...
class IceCubeDataset(Dataset):
    ...
    def _load_hdf5_event(
        self, filepath: str, event_idx: int
    ...
    def _load_parquet_event(
        self, filepath: str, event_id: Union[int, str]
    ) -> Tuple[np.ndarray, np.ndarray, int]:
        ...
        if all(col in df.columns for col in ["x", "y", "z"]):
            ...
            raise ValueError(
                "Parquet file must contain 'x', 'y', 'z' columns or 'positions' column"
            )
    ...
    def _augment(
        self, positions: np.ndarray, features: np.ndarray
