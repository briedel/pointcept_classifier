...
class PointceptClassifier(nn.Module):
    ...
    def _create_simple_backbone(self) -> nn.Module:
        ...
        return SimplePointNet(
            in_channels=3 + self.in_channels, out_channels=self.hidden_dim
        )

    def forward(
        self, coord: torch.Tensor, feat: torch.Tensor, **kwargs
    ) -> torch.Tensor:
        """
        Forward pass.
        ...
        try:
            ...
            features = self.backbone(coord=coord, feat=feat, **kwargs)
        except TypeError:
...
class SimplePointNet(nn.Module):
    ...
    def __init__(
        self, in_channels: int = 4, out_channels: int = 256  # 3 coords + 1 feature
    ):
        super().__init__()
...
def build_pointcept_classifier(
    model_name: str = "simple", num_classes: int = 3, in_channels: int = 1, **kwargs
) -> PointceptClassifier:
    """
