...
def example_training():
    ...
    model = build_pointcept_classifier(
        model_name="simple", num_classes=3, in_channels=2, hidden_dim=256
    )
...
def example_inference():
    ...
    # Create model
    model = build_pointcept_classifier(
        model_name="simple", num_classes=3, in_channels=2
    )
    model.eval()
...
def example_custom_preprocessing():
    ...
    positions, features = icecube_to_pointcloud(
        raw_data, feature_names=["time", "charge"]
    )
    ...
    positions_norm, features_norm = preprocess_event(
        positions, features, normalize=True, remove_noise=True, noise_threshold=1.0
    )
...
def example_batch_prediction():
    ...
    for i, (pred, prob, true_label) in enumerate(
        zip(predictions, probabilities, batch["label"])
    ):
        print(
            f"  Event {i}: pred={dataset.class_names[pred]} "
