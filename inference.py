...
def main():
    ...
    parser.add_argument(
        "--data_path", type=str, required=True, help="Path to test data"
    )
    parser.add_argument(
        "--output_dir", type=str, default="outputs", help="Output directory"
    )
    parser.add_argument(
        "--split", type=str, default="test", help="Data split to evaluate"
    )
    ...
    # Setup logger
    logger = setup_logger(
        name="inference", level=logging.INFO, log_file=output_dir / "inference.log"
    )
    ...
    for class_name, metrics in results["class_metrics"].items():
        logger.info(
            f"  {class_name}: {metrics['accuracy']:.2f}% ({metrics['count']} samples)"
        )

    logger.info("\nConfusion Matrix:")
    logger.info(
        "           " + "  ".join([f"{name:>10}" for name in dataset.class_names])
    )
    for i, row in enumerate(results["confusion_matrix"]):
        logger.info(
            f"{dataset.class_names[i]:>10} " + "  ".join([f"{val:>10}" for val in row])
        )
